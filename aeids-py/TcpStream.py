...
def generate_id(ether):
    ...
    if isinstance(segment, ImpactPacket.TCP):
        ...
        return (
            "{}-{}-{}-{}-{}".format(src_addr, src_port, dst_addr, dst_port, protocol),
...
def generate_reverse_id(ether):
    ...
    if isinstance(segment, ImpactPacket.TCP):
        ...
        dst_port = segment.get_th_dport()
        return (
            "{}-{}-{}-{}-{}".format(dst_addr, dst_port, src_addr, src_port, protocol),
            (src_addr, src_port, dst_addr, dst_port),
        )
    else:
        return False
...
class TcpStream(threading.Thread):
    ...
    def run(self):
        while self.state not in end_states:
            if (
                self.reader_thread.is_timeout(self.last_packet_time)
                and self.state not in end_states
            ):
    ...
    def add_packet(self, ts, to_server, ether):
        ...
        if segment.get_SYN() and to_server:
            ...
            segment.get_ACK()
            and self.state == STATE_WAIT_FIN2
            and to_server
            and segment.get_th_seq() > self.server_last_seq
        ):
            # print("close1")
            self.last_packet_time = ts
            self.stop_time = ts
            ...
            and self.state == STATE_WAIT_FIN2
            and not to_server
            and segment.get_th_seq() > self.client_last_seq
        ):
            # print("close2")
            self.last_packet_time = ts
            self.stop_time = ts
                ...
                if len(segment.get_data_as_string()) > 0:
                    ...
                    if to_server:
                        if self.server_last_seq < segment.get_th_seq():
                            ...
                            # print(8)
                            for i in range(
                                0, len(self.server_buffer)
                            ):  # check for retransmission
                                segment_tuple = self.server_buffer[i]
                                if (
                                    segment_tuple[0] == segment.get_th_seq()
                                    and segment_tuple[1] == segment.get_th_ack()
                                    and len(segment_tuple[2])
                                    == len(segment.get_data_as_string())
    ...
    def finish(self):
        ...
        self.client_bf = __calculate_byte_frequency__(
            self.client_data, self.client_data_len
        )
        self.server_bf = __calculate_byte_frequency__(
            self.server_data, self.server_data_len
        )
